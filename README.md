...
Art Direction
Silhouette rendering: rigged 3D humanoid models (Synty Studios assets) rendered as pure black silhouettes, set against colored, gradient backdrops hand-painted by me with my graphic tablet, one per zone — full value-contrast keeps gameplay and the hero readable.
...
Tone: the silhouette style is a purely visual/technical choice — it does not set the narrative tone. Emberhood is beautiful, lighthearted, and inviting, closer to Swordigo and Donkey Kong Country Returns' vibrant silhouette levels than to Limbo/Inside's bleak atmosphere.
...
References: Inside for the 3D-rig-rendered-as-silhouette pipeline (not Limbo, which uses pure 2D sprites), Bad Apple!! for expressive movement in pure silhouette, and Donkey Kong Country Returns' silhouette levels (Sunset Shore, Foggy Fumes, Smokey Peak) as proof that fully-colored, high-contrast backdrops stay readable behind pure black silhouettes.
World

Four zones leading into a final confrontation:

Canyon désertique crépusculaire → Zone normale → Jungle → Grotte de cristal bioluminescente → Boss zone

Each zone (plus the final boss zone) has its own hand-painted backdrop — 5 in total. A merchant hub sits somewhere along the route (tentatively between the jungle and the crystal cave). An intermediate boss caps the end of Zone normale, ahead of the final boss.
